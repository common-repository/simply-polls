...
function alex_sp_get_all_polls(){
	global $wpdb;
...
function alex_sp_add_poll($title, $content){
	global $wpdb;
	$pagination_params = alex_sp_pagination_params();
...
function alex_sp_add_answs($answs){
	global $wpdb;
...
function alex_sp_del_one_poll($id){
	global $wpdb;
...
function alex_sp_edit_answs($poll_id, $answs){
	global $wpdb;
	foreach($answs as $k => $v) {
		if( !is_int($k)) {
		   	$wpdb->insert( 
...
function alex_sp_add_ip($ip, $poll_id){
	global $wpdb;
	$add_poll = $wpdb->insert( 
		$wpdb->table3_name, 
		array( 
...
function alex_sp_check_ip($ip, $poll_id){
	global $wpdb;
