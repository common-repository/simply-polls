...
function alex_sp_cb_polls_options(){
    ...
    <?php
    	if($_POST['add']) {
    		...
    		// echo "</pre>";
    		if( !empty($title) ){
    			$add_poll = alex_sp_add_poll($title, $content);
    		}else{
    			$error['title'] = 1;
    		}

    		alex_sp_add_answs($answs);
	...
	case 'results':
		...
		$title = alex_sp_get_one_poll_title($poll_id);
		$answers = alex_sp_get_one_poll_answers($poll_id);
		if(!empty($title)){
	...
	case 'edit':
    		if($_POST['edit']) {
	    		...
	    		$title = sanitize_text_field($_POST['title']);
	    		$content = sanitize_text_field($_POST['content']);
		    	alex_sp_edit_poll($poll_id, $title, $content);
				...
				alex_sp_edit_answs($poll_id, $answs);
	    	}
	...
	default:
			...
			$all_polls = alex_sp_get_all_polls();
			?>
			<?php if($all_polls):?>		
					<table class="wp-list-table widefat fixed striped posts">
						...
						<tbody id="the-list">					
							...
							<?php
								$answers = alex_sp_get_one_poll_answers($poll['id']);
								// print_r($answers);
								$count_votes = 0;
								foreach ($answers as $k => $v) {
									$count_votes = $count_votes + $v['votes'];
...
function alex_sp_action_callback() {
		...
		if( (int)$_POST['poll_id'] > 0) {
		   alex_sp_del_one_poll( $_POST['poll_id'] );
		 }
		wp_die();
...
function alex_sp_cb_del_one_answ() {
		...
		if( (int)$_POST['answ_id'] > 0 and (int)$_POST['poll_id'] > 0 ) {
		   alex_sp_del_one_answ( $_POST['poll_id'], $_POST['answ_id'] );
		 }
		wp_die();
...
function alex_sp_cb_poll( $atts ) {
   ...
   $poll_id = (int)$atts['id'];
   $check_ip = alex_sp_check_ip($ip, $poll_id);
...
function alex_sp_handler_ajax(){
	...
	alex_sp_add_vote($poll_id, $answ_id);
	$ip = $_SERVER['REMOTE_ADDR'];
	alex_sp_add_ip($ip, $poll_id);
